...
def main():
    ...
    permitted = {
        ...
        'math_invert': ('aciarena/attacks/hijacking_attack.py', 'MathInvertMessage'),
    }
    ...
    namespace = dict(parse=parse, verify=verify, ExprExtractionConfig=ExprExtractionConfig,
                     LatexExtractionConfig=LatexExtractionConfig, FiniteSet=FiniteSet,
                     Interval=Interval, latex=latex, Basic=Basic, Equality=Equality,
    ...
    for method in cls.body:
        if isinstance(method, ast.FunctionDef) and method.name in (
                'extract_answer', 'apply_mapping', 'is_applicable', 'invert_sign', 'verify'):
