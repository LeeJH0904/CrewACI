...
- **D48 — 게이트별 오케스트레이터 통합(G6 착수 시 구현 완료):** 실행
  ...
  - **통합 오케스트레이터가 남기는 책임(②③④ 유지 / ① 제거, 2026-09-13):** `run_g5_matrix.py`
    부피는 실행이 아니라 오케스트레이션이며, 그 책임은 두 부류로 갈린다. **① 돈을 다루는
    비용 피드백 정지 루프**(배치 사이 누적 비용 재계산 → $2 실링·세션 한도·usage 불완전 halt)는
    위 "방지턱 제거" 대상이므로 통합 시 **버린다**(dry-run 기본값만 잔존). 반면 **② config_hash
    기반 재개·중복방지**(완료 셀 skip을 config_hash가 일치할 때만 인정 → 유료 재실행 중복지불
    방지), **③ 결정적 매트릭스 열거**(stage·도메인별 공격 매핑·`not_applicable` 규칙으로
    1056셀 계획), **④ 진행 스냅샷 + 종단 `audit_matrix` 인증**(스테이지 완결·유효성 증명)은
    "돈"이 아니라 **실험 정합성**의 문제이므로 **유지**한다. 통합 `run_experiment.py`는 ①을
    제거한 채 ②③④를 `--stage`/`--matrix`/`--report gate|progress` 모드로 흡수하고, 단일 셀
    실행(현 `benchmark.py` 경로)과 매트릭스 스윕을 같은 엔진 위 두 진입 모드로 통일한다.
    셸 루프는 ②③④를 재현할 수 없어(누적 상태 파싱·config_hash 인지·감사 부재) 대체가 아니다.
  - **개선안 — manifest 기반 `--suite` 계열 확장을 CLI에 노출(2026-09-13):**
    - **문제제기(사용자):** recorded 경로는 attack 실행 시 `--attack_ids`를 강제한다
      (`recorded_executor.py`: attack_ids 없고 suite≠benign이면 에러). 그런데 attack_id는
      `카테고리.표면.변형`이고 goal은 `category.split('_')[0]`로 파생되므로(catalog/records
      불변식), 공격 실행에서 `--suite`는 새 정보 없이 goal 일치만 재확인하는 잉여 가드다.
      그렇다면 (a) 레거시는 공격 "종류(계열) 단위"로만 돌렸고 지금은 "개별 단위"로 내려온 것,
      (b) 반대로 지금은 개별 지정이 강제되어, "hijacking 계열 전체" 벤치를 하려면 ID를
      일일이 나열하거나 셸 스크립트를 짜야 하는 것 아니냐는 문제제기.
    - **답변:** "계열 통째 실행"은 제거된 게 아니라 **정의처가 이동**했다. `run_g5_matrix.g5_groups`가
      이미 `spec.goal == suite`로 **동결 manifest catalog에서 계열 전체를 자동 열거**한다
      (레거시의 가변 코드 레지스트리 확장 → manifest 기반 결정적 확장). 즉 사용자가 ID를
      나열하거나 셸 루프를 짤 필요 없이 "계열 전체"는 이미 일급 기능이며, 다만 그 확장이
      오케스트레이터 내부에만 있어 `benchmark.py` 직접 호출에선 안 보였을 뿐이다. 강제화의
      진짜 이유는 재현성 — "지금 등록된 hijacking 전부"식 레지스트리 확장은 공격 클래스가
      하나 추가되면 벤치 집합이 조용히 바뀌어 config_hash를 흔들기 때문이다.
    - **결정:** 통합 `run_experiment.py`에서 이 계열 확장을 **CLI에 정식 노출**한다 —
      `--suite <goal>`을 "동결 manifest에서 `goal==<goal>`이고 해당 도메인에 적용되는 attack_id
      전부"로 **결정적 확장**하고, `--attack_ids`는 그보다 세밀한 부분집합 지정으로 공존시킨다
      (attack_ids 우선, 미지정 시 suite 확장, benign은 attack='none'). 이로써 레거시의 편의성
      (계열 한 단어)과 현재의 결정성(해시 고정)을 **둘 다** 얻고, "개별밖에 못 돌린다"는 제약도
      해소한다. 확장 기준은 반드시 레지스트리가 아니라 manifest여야 provenance가 유지된다.
